...
PostProcessModel <- function(){
  ...
  if (model.results$is.pp) {
    #if (!model.results$is.pp)
    model.results$pp.model <- as.data.frame(model.results$model)
    
    n.col.to.add <- length(model.results$pp.eqns.col)
   #observe({print(n.col.to.add)})
    
    for (i in seq(n.col.to.add)) {
      new.col <- eval(parse(text = model.results$pp.eqns.col[i]))
      #observe({print(head(new.col))})
      eval(parse(text = paste0("model.results$pp.model$",model.results$pp.vars[i], "<-new.col")))
    }
  }
}
...
observeEvent(input$pp_submit_new_var, {
  ...
  model.results$pp.vars <- c(model.results$pp.vars, input$pp_new_var)
  model.results$pp.eqns <- c(model.results$pp.eqns, ppEqnToAdd())
  model.results$pp.eqns.col <- c(model.results$pp.eqns.col, ppEqnToAddColumn())
  PostProcessModel()
  
  model.results$is.pp = TRUE
...
ModelToUse <- reactive({
  if (model.results$is.pp) {
    ...
    model.to.use <- model.results$pp.model
  }else{
    observe({print("is.pp is false - normal model")})
    model.to.use <- model.results$model
  }
